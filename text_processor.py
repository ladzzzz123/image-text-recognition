...
def label_checker(label_to_check, label_from_text, dest ):
    ...
    max_itr = min(len(str(label_from_text[0])), len(sorted_label[0][0]))
    flag_size = 1
    while flag_size <= max_itr:
        to_pop_label = []
        to_pop_text = []
        for i in range(0, len(sorted_label)):
            for j in range(0, len(label_from_text)):
                if len(sorted_label[i][0]) == len(str(label_from_text[j])) \
                        and label_cmp(sorted_label[i][0], str(label_from_text[j]), flag_size):
                    to_pop_label.append(i)
                    to_pop_text.append(j)
                    sorted_label[i][0] = str(label_from_text[j])
                    print(", ".join(sorted_label[i]), file=fp_w)
                    break
        indexes = sorted(to_pop_label, reverse=True)
        for index in indexes:
        ...
        for index in indexes:
            del label_from_text[index]
        flag_size += 1
        if len(sorted_label) == 0 or len(label_from_text) == 0:
            break



def label_cmp(img_label, text_label, flag_size):
    score = 0
    flag = flag_size
    for i in range(0, len(img_label)):
        if img_label[i] == text_label[i]:
            ...
            if img_label[i] == '7' and text_label[i] == '1':
                score += 1
